...
from src.auth import verify_email, login_user, register_user
from src.constants import MAP_ACTIVITY, MAP_OBJETIVES, MAP_SEX
...
def render():
    ...
    with tab_login:
        ...
        with st.form("form_login"):
            ...
            passwd_login = st.text_input("Contraseña:", type="password")
    ...
    with tab_register:
        ...
        with st.form("form_register"):
            ...
            passwd_register = st.text_input("Contraseña:", type="password")

            st.divider()

            st.text("Información personal")
            sex = st.selectbox(
                "Sexo:", options=list(MAP_SEX.keys()), format_func=lambda x: MAP_SEX[x]
            )
            age = st.number_input("Edad:", min_value=12, max_value=90, value=18, step=1)
            weight = st.number_input(
                "Peso (kg):", min_value=10.0, max_value=250.0, value=70.0, step=0.01
            )
            height = st.number_input(
                "Altura (cm)", min_value=90, max_value=230, value=170, step=1
            )
            ...
            register_button = st.form_submit_button("Registrarse", type="primary")
            ...
            if register_button:
                if verify_email(email_register):
                    ...
                    if user_id is None:
                        st.caption(
                            "Error del sistema. Inténtelo de nuevo más adelante..."
                        )
                    else:
                        st.session_state["user_id"] = user_id
