    ...
    with connect_db() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                cursor.execute(query, (email, password_hash.decode('utf-8'), sex, activity, objective, height, age, weight))
...
def login_user(
    ...
    try:
        with connect_db() as connection:
            with connection.cursor() as cursor:
                ...
                if bcrypt.checkpw(passwd.encode('utf-8'), passwd_hash.encode('utf-8')):
                    logger.info(f"Nuevo inicio de sesión ({email})")
                    return user_id
                else:
        ...
        logger.error(f"Error en la base de datos al consultar información sobre usuario.")
        return None
